...
explicit OpenAI-compatible endpoint. The API key is held only in the current
Studio server process and is not written to the Studio database. Clearing the
provider or stopping Studio removes it.
...
- **Plan** can prepare only policy-bounded candidates. It pauses at a final card
  containing the exact parameters and input snapshot; computation starts only
