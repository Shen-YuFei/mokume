...
def generate_de_report(
    ...
    # Build protein intensity data as JSON for JavaScript
    intensity_data = {}
    for _, row in protein_df.iterrows():
        ...
        for s in exp_samples:
            ...
            if pd.notna(val) and val > 0:
                intensities[s] = float(val)
        if intensities:
            intensity_data[str(prot)] = intensities
