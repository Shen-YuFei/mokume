...
def load_dataset(
    ...
    *,
    ...
    # Expand intensities — vectorized via explode
    feat = feat[
    ...
    if is_tmt:
        # GIS normalization — per-run, accession-based
        # First merge long with run_map to get sample_accession per row
        ...
        if not gis_accessions:
            # Fallback: use TMT126/TMT131 labels
        ...
        if gis_data.empty:
            ...
            # Mean GIS intensity per (protein, run)
            gis_mean = (
    ...
    # Protein-level summarisation (sum peptide intensities)
    prot_samp = (
    ...
    meta["batch"] = meta.index.map(batch_map).fillna("unknown")
    ...
    # Warn about low-sample tissues (only when threshold > 0)
