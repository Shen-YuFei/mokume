...
from mokume.agentic.state import AgenticState, CandidateConfig, EvaluationResult, RoundResult
...
def test_save_outputs_creates_files():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        profile = DataProfile(
            n_proteins=100, n_samples=6, n_conditions=2,
            samples_per_condition={"A": 3, "B": 3},
            missing_rate=0.1, missing_per_sample={},
            median_cv=0.12, has_peptide_counts=False,
            data_type="LFQ", batch_fields=[],
            intensity_range=(10.0, 25.0), is_log_transformed=True,
        )
        ...
        state = AgenticState(
            ...
            rounds=[RoundResult(
                ...
                results=[EvaluationResult(
                    config_name="best", config=best_cfg.to_dict(),
                    tp=47, fp=1, auc=0.996, n_de_up=100, n_de_down=20,
                    score=0.95,
                )],
