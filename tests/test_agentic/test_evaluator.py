...
def test_evaluate_with_ground_truth(
    synthetic_protein_df, sample_to_condition, ground_truth_proteins,
):
    ...
    result = evaluate(
        cfg, de_df, synthetic_protein_df, sample_to_condition, ground_truth_proteins,
    )
...
def test_score_ground_truth():
    ...
    r = EvaluationResult(
        config_name="t", config={}, tp=47, fp=1, auc=0.996,
    )
    score = compute_score_ground_truth(r, max_tp=48, total_tested=1300, weights=ScoreWeights())
    assert score > 0.7
...
def test_score_unsupervised():
    ...
    r = EvaluationResult(
        config_name="t", config={}, n_de_up=200, n_de_down=50,
        median_cv=0.15, missing_rate=0.10,
    )
