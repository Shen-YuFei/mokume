...
class QpxFeatureAdapter:
    ...
    def get_median_map_to_condition(self) -> dict[str, dict[str, float]]:
        ...
        grp = (
            df.groupby(["condition", "sample_accession"])["intensity"]
            .median()
            .unstack(level=0)
        )
        med_map = {}
    ...
    def get_run_to_fraction(self) -> Optional[dict[str, str]]:
        ...
        for _, row in run_df.iterrows():
            ...
            frac_str = (
                str(frac).strip() if pd.notna(frac) and str(frac).strip() else "1"
            )
            out[fname] = frac_str
    ...
    def enrich_with_sdrf(self, sdrf_path: str) -> None:
        ...
        logger.debug(
            "enrich_with_sdrf is a no-op when using QPX directory; metadata from qpx"
        )
    ...
    def get_report_from_database(
        self, samples: list, columns: Optional[list] = None
    ) -> pd.DataFrame:
    ...
    def get_low_frequency_peptides(self, percentage: float = 0.2) -> tuple:
        ...
        df = _apply_filter_builder(df, self.filter_builder)
        grp = (
            df.groupby(["sequence", "pg_accessions"])["sample_accession"]
            .nunique()
            .reset_index()
        )
...
def _apply_filter_builder(df: pd.DataFrame, filter_builder) -> pd.DataFrame:
    ...
    if (
        getattr(filter_builder, "min_peptide_length", 0) > 0
        and "sequence" in out.columns
    ):
    ...
    if (
        getattr(filter_builder, "remove_contaminants", True)
        and "pg_accessions" in out.columns
    ):
        patterns = getattr(
            filter_builder, "contaminant_patterns", ["CONTAMINANT", "ENTRAP", "DECOY"]
        )
        for pat in patterns:
            out = out[
                ~out["pg_accessions"]
                .astype(str)
                .str.contains(pat, regex=False, na=False)
            ]
    return out
...
def create_feature_for_input(
    ...
    if parquet:
        ...
        return QpxFeatureAdapter(
            _SingleFeatureDataset(feat), filter_builder=filter_builder
        )
