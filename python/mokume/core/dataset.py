...
def _extract_scalar(val):
    ...
    """
    if val is None or (
        hasattr(val, "__class__") and val.__class__.__name__ == "NAType"
    ):
        return None
...
class QpxDataset:
    ...
    def subset_samples(self, sample_ids: List[str]) -> "QpxDataset":
        ...
        new = QpxDataset(
            ...
            layers={},
            protein_info=self.protein_info.copy()
        ...
        for level_name in self._VALID_LEVELS:
            ...
            if df is not None and SAMPLE_ID in df.columns:
                ...
                non_sample_cols = [
                    c for c in df.columns if c not in sample_set and c not in sample_ids
                ]
                if sample_cols:
    ...
    def subset_proteins(self, protein_ids: List[str]) -> "QpxDataset":
        ...
        for key, layer_df in self.layers.items():
            if PROTEIN_NAME in layer_df.columns:
                new.layers[key] = layer_df[
                    layer_df[PROTEIN_NAME].isin(protein_set)
                ].copy()
            else:
                new.layers[key] = layer_df.copy()
    ...
    def save(self, directory: str) -> None:
        ...
        for level_name in self._VALID_LEVELS:
            ...
            if raw is not None:
                df = _ensure_df(raw)
                df.to_parquet(
                    os.path.join(directory, f"{level_name}.parquet"), index=False
                )
        ...
        if layer_names:
            for layer_name in layer_names:
                if layer_name in self.layers:
                    ...
                    if (
                        sample_col in layer_df.columns
                        and protein_col in layer_df.columns
                    ):
    ...
    def __repr__(self) -> str:
        ...
        for level_name in self._VALID_LEVELS:
            ...
            if _is_lazy(raw):
                ...
                parts.append(
                    f"  {level_name}: LazyFrame ({ncols} cols, not materialized)"
                )
            else:
                parts.append(
                    f"  {level_name}: {raw.shape[0]} rows x {raw.shape[1]} cols"
                )
        if self.sample_info is not None:
