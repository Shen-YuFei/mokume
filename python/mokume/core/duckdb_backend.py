...
class LazyFrame:
    ...
    def from_parquet(
        ...
        relation = connection.sql(f"SELECT * FROM parquet_scan('{safe_path}')")
    ...
    def filter(self, condition: str) -> "LazyFrame":
        ...
        return LazyFrame(
            new_relation, self._connection, source=f"{self._source}[filtered]"
        )
    ...
    def select(self, *columns: str) -> "LazyFrame":
        ...
        *columns : str
        ...
        return LazyFrame(
            new_relation, self._connection, source=f"{self._source}[select]"
        )
