...
def run_proda(
    ...
    # proDA tests each coefficient against the empirical-Bayes-moderated df the
    # fit already computes per protein (``fit["df"]`` = variance-prior df0 +
    # residual df), not the naive ``n_samples - p`` -- the moderated df is what
    # makes the test less conservative on small samples. ``df -> inf`` is proDA's
    # fully-moderated limit (a normal test); a large finite df reproduces it.
    fallback_df = float(max(1, n_samples - X.shape[1]))
    results = []
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        for i, gene in enumerate(gene_names):
            ...
            moderated_df = float(fit["df"])
            if not np.isfinite(moderated_df):
                moderated_df = 1e6
            elif moderated_df <= 0:
                moderated_df = fallback_df

            if se_fc < 1e-10 or np.isnan(se_fc) or np.isnan(log2fc):
                t_stat, pvalue = 0.0, 1.0
            else:
                t_stat = log2fc / se_fc
                pvalue = 2.0 * float(t_dist.sf(abs(t_stat), moderated_df))
