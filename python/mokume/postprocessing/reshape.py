...
def remove_samples_low_protein_number(
    ...
    logger.info(
        "The number of samples with number of proteins lower than %d is %d",
        min_protein_num,
...
def remove_missing_values(
    ...
    logger.info(
        "Number of samples removed: %d", initial_sample_count - final_sample_count
    )
...
def pivot_longer(
    ...
    if long_df[values].isna().any():
        logging.warning(
            "Found %d missing values in the result",
            long_df[values].isna().sum(),
        )
